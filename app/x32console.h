...
public:
    Mutegroup(qint16 number, bool state = false, QObject *parent = nullptr) :
        QObject(*parent), number(number), state(state) {
...
public:
    Channel(QObject *parent = nullptr) : QObject(*parent) {
    }
...
class Mutegroup : public QObject {
    ...
    Mutegroup(QObject *parent = nullptr);
};
